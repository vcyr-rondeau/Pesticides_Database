...
class PesticideApp:
    def __init__(self, root, data):
        ...
        # Sort pesticides alphabetically (case-insensitive)
        self.data = sorted(
            data,
            key=lambda pesticide: pesticide["name"].casefold()
        )
        ...
        # Populate the listbox in alphabetical order
        for pesticide in self.data:
            self.listbox.insert(tk.END, pesticide["name"])
    ...
    def on_select(self, event):
        ...
        file_name = os.path.join(
            image_folder,
            f"{pesticide['name'].replace(' ', '_')}.png"
        )
